...
function isPalindrome(str) {
  const stroke = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();

  return stroke === stroke.split('').reverse().join('');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < characters.length; i += 1) {
    ...
    if (currentChar === currentChar.toUpperCase()) {
      characters[i] = currentChar.toLowerCase();
    } else {
      characters[i] = currentChar.toUpperCase();
    }
  }
